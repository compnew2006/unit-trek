...
export const createMovementSchema = z.object({
  ...
  type: z.enum(['in', 'out', 'transfer'], {
    message: 'Type must be "in", "out", or "transfer"'
  }),
...
export const validate = (schema: z.ZodSchema) => {
  return (req: Request, res: Response, next: NextFunction): void => {
    try {
      ...
      // Update request with validated data (merge only object types)
      if (typeof validated === 'object' && validated !== null) {
        const validatedObj = validated as Record<string, unknown>;
        req.body = { ...req.body, ...validatedObj } as typeof req.body;
        // Params and query are typically read-only, so we'll just validate
        // They can be accessed via req.params and req.query directly
      }
